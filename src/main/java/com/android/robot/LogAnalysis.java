...
public class LogAnalysis {
	...
	public void createLogList(){
		...
		CTabItem tabItemLogList = new CTabItem(tabFolderLogList, SWT.NONE|SWT.MULTI|SWT.V_SCROLL);
		...
		treeLog.addListener (SWT.MouseDoubleClick, new Listener () {
			public void handleEvent (Event event) {
			    ...
			    if(item != null){
			    	...
			    	if(index != 0)
			    		Log.loadLogs(styledTextLog, display,logFile,taskName,loop,caseName, index);
			    }
			}
		});
	...
	public void createLogDetail(){
		...
		CTabItem tabItemLogList = new CTabItem(tabFolderLogDetail, SWT.NONE|SWT.MULTI|SWT.V_SCROLL);
		tabFolderLogDetail.setSelection(tabItemLogList);
		
		//styledTextLog = new List(tabFolderLogDetail, SWT.BORDER);
		styledTextLog = new StyledText(tabFolderLogDetail, 
