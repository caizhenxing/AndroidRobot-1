...
//	}
	...
	private void createTable(Composite rtComp){
		...
		: (MemInfo.class$org$jfree$data$time$Millisecond = MemInfo.getClass("org.jfree.data.time.Millisecond")));
	...
	public int getPSSByALL(String memInfo) {
		int pss = 0;
		int index = memInfo.indexOf("Total PSS:");
		//int index = memInfo.indexOf("Used RAM:");

		if (index >= 0) {
			String line = memInfo.substring(index, memInfo.length());
			line = line.substring(0, line.indexOf("\n"));
			String test[] = line.split("\\s+");
			pss = Integer.parseInt(test[2]);// PSS
		}
		return pss;
	}
	...
	public int getPSSByPID(String memInfo) {
		int pss = 0;
		int index = memInfo.indexOf("TOTAL");
		if (index >= 0) {
			String line = memInfo.substring(index, memInfo.length());
			line = line.substring(0, line.indexOf("\n"));
			String test[] = line.split("\\s+");
			...
			try {
				synchronized(this){
					...
					if(!currentDevice.equals("") && !isOpened){
						String DUMPSYS_MEMINFO = "adb -s "+currentDevice+" shell dumpsys meminfo";
